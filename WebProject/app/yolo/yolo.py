...
class YOLO(object):
    ...
    def detect_image(self, image):
        ...
        with graph.as_default():
            ...
            # remove the almost same boxes (when iou >0.5) with lower score.
            # remove_index = []
    ...
    def detect_image_evaluate(self, image, single_image=True, output=list()):
        start = timer()
...
def detect_camera(yolo):
    ...
    # if isOutput:
    #     print("!!! TYPE:", type(output_path), type(video_FourCC), type(video_fps), type(video_size))
    #     out_1_before = cv2.VideoWriter(output_path, video_FourCC, video_fps, video_size)
    accum_time = 0
    curr_fps = 0
    fps = "FPS: ??"
    prev_time = timer()
